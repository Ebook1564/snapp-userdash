    ...
    const fetchProfile = async () => {
      ...
      try {
        const res = await fetch("/api/user/profile", {
          ...
          // ask for debug rawRow in non-production so we can inspect actual DB columns
        ...
        if (!res.ok) {
          ...
          // merge mapped user + rawRow (if present) so UI can access whichever field contains phone
          const merged = { ...(data.user || {}), ...(data.rawRow || {}) };
          console.debug("[UserInfoCard] merged profile:", merged);
          setProfile(merged);
        }
      } catch (err) {
      ...
      <div className="flex flex-col gap-6 lg:flex-row lg:items-start lg:justify-between">
        <div>
          ...
          <div className="grid grid-cols-1 gap-4 lg:grid-cols-2 lg:gap-7 2xl:gap-x-32">
            {loading ? (
              <div className="col-span-1 lg:col-span-2 text-sm text-gray-500">Loading profile...</div>
            ) : error ? (
              <div className="col-span-1 lg:col-span-2 text-sm text-red-500">{error}</div>
            ) : (
              <>
                <div>
                  <p className="mb-2 text-xs leading-normal text-gray-500 dark:text-gray-400">First Name</p>
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{(profile.username || profile.user_name || profile.first_name) || clientName || "-"}</p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{profile.useremail || profile.email || clientEmail || "-"}</p>
                </div>
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">
                    {(() => {
                      ...
                      phone = phone.trim();
                      if (!phone) return "-";
                      // if phone already includes country code, avoid duplication
                      const normalized = phone.replace(/^\+/, "");
                      const normalizedCode = code.toString().replace(/^\+/, "");
                      if (normalized.startsWith(normalizedCode) && normalizedCode) {
                        return `${phone}`;
                      }
                ...
                <div>
                  ...
                  <p className="text-sm font-medium text-gray-800 dark:text-white/90">{profile.country_name || "-"}</p>
                </div>
