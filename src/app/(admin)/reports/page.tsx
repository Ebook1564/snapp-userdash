...
export default function ReportsPage() {
    ...
    const { summary, timeRanges, chartData, tableData } = REPORT_DATA;

    const handleDownloadCSV = () => {
        downloadCSV(tableData, `report-${activeRange.replace(/\s+/g, '-').toLowerCase()}`);
    };

    // Simulate data variation based on time range and selected metric
    ...
    const chartOptions: import("apexcharts").ApexOptions = {
        ...
        xaxis: {
            ...
            labels: {
                ...
                formatter: (val: string, index: number) => {
                    const step = Math.ceil(currentChartData.length / 6);
        ...
        yaxis: {
            min: 0,
            max: activeMetric === "impressions" ? 200 : activeMetric === "ecpm" ? 1.0 : 0.15,
            tickAmount: 3,
            labels: {
    ...
    const chartSeries = [
        {
            ...
            data: currentChartData.map((d) => d.value),
        },
    ];
                        ...
                        return (
                            ...
                            >
                                ...
                                <p className="mt-1.5 text-xl font-bold tracking-tight">
                                    {isActive && activeMetric === "revenue" && (currentChartData[0]?.value === 0) ? "--" : item.value}
                                </p>
                            </div>
                        );
                    })}
                    ...
                    <table className="min-w-full text-xs">
                        <thead>
                            <tr className="bg-gray-50/50 dark:bg-white/[0.02]">
                                <th className="py-2.5 px-4 text-left font-bold text-gray-600 dark:text-gray-400 border-r border-gray-100 dark:border-gray-800 w-48">Date</th>
                                <th className="py-2.5 px-4 text-left font-bold text-gray-600 dark:text-gray-400 border-r border-gray-100 dark:border-gray-800">
                                ...
                                <th className="py-2.5 px-4 text-left font-bold text-gray-600 dark:text-gray-400">
                                    <div className="flex items-center gap-1">
                                        eCPM
                                        <svg className="w-3 h-3 text-gray-400" viewBox="0 0 20 20" fill="currentColor">
                                            <path fillRule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clipRule="evenodd" />
                        ...
                        <tbody className="divide-y divide-gray-100 dark:divide-gray-800">
                            {tableData.map((row, idx) => (
                                <tr key={idx} className={`${idx === 0 ? "font-bold bg-white dark:bg-white/[0.03]" : ""} hover:bg-gray-50/50 dark:hover:bg-white/[0.02] border-b border-gray-100 dark:border-gray-800 transition-colors`}>
                                    <td className="py-2 px-4 text-gray-900 dark:text-white/90 border-r border-gray-100 dark:border-gray-800">{row.date}</td>
                                    <td className="py-2 px-4 text-gray-700 dark:text-gray-300 border-r border-gray-100 dark:border-gray-800">{idx === 0 && activeMetric === "revenue" && row.revenue === "-" ? "--" : row.revenue}</td>
                                    <td className="py-2 px-4 text-gray-700 dark:text-gray-300 border-r border-gray-100 dark:border-gray-800">{row.impressions}</td>
                                    <td className="py-2 px-4 text-gray-700 dark:text-gray-300">{row.ecpm}</td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
